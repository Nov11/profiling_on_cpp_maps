...
结论是linux和windows上都是1ns。之前测试的结果均以微妙为单位，因此结果不受时钟精度影响。原以为精度是1us，没想到精度这么高。

2. 在linux下，针对数据量10000，每个数据一次插入，一次主键查找，5次范围查找，一次删除的情况做测试。

根据操作频度修改了测试函数test，产生测试数据的时候加了shuffle，这样用例id更贴合实际情况，详情见test.cpp。具体测试方法是运行order程序100次，统计哪种方案速度最快的次数最多。结果显示avl最好，90%以上的情况avl最快，即使用map和multimap组合。测试脚本在prof/build下。其中runtest.sh执行测试，结果输出到result文件。如需扩大测试重复次数，可修改runtest.sh。编译选项还是O3优化。
